...
let Menu = Menu_1 = class Menu extends UI5Element {
    ...
    _closeAll() {
        let menu = this, parentMenuItem;
        do {
            if (menu) {
                menu.close();
                parentMenuItem = menu._getParentMenuItem(menu);
            }
            menu = parentMenuItem ? parentMenuItem.parentElement : menu;
    ...
    _createSubMenu(item, opener) {
        ...
        if ((!subMenu && !this._isSubMenu) || (!subMenuRef && this._isSubMenu)) {
            subMenu = document.createElement(this.tagName.toLowerCase());
    ...
    _getSubmenuReference(item) {
        const index = item.parentElement?.items.indexOf(item);
        return this.staticAreaItem
            ? this.staticAreaItem.shadowRoot.querySelector(`.ui5-menu-submenus > ui5-menu[id$=menu-item-${index}]`)
            : undefined;
    }
    ...
    async _openItemSubMenu(item, opener) {
    ...
    _closeItemSubMenu(item, forceClose = false, keyboard = false) {
        if (item) {
            const subMenu = item._subMenu
                ? item._subMenu
                : item.parentElement?._getSubmenuReference(item);
            if (forceClose) {
    ...
    async _prepareSubMenu(item, opener) {
    ...
    _itemKeyDown(e) {
        ...
        if (shouldOpenMenu) {
            ...
            const parentMenuItemMenu = parentMenuItem.parentElement;
            ...
            if (!this._isSubMenu) {
                ...
                if (!prevented) {
                    ...
                    do {
                        ...
                        openerMenuItem = parentMenu._getParentMenuItem(parentMenu);
    ...
    _findMainMenu(element) {
        let menu = this._isMenu(element) ? element : element.parentElement, parentMenuItem;
        do {
    ...
    _getParentMenuItem(element) {
        if (!element) {
            return;
        }
        const menu = this._isMenu(element) ? element : element.parentElement;
        return !menu._parentMenuItem || menu._parentMenuItem.parentElement
            ? menu._parentMenuItem
            : menu._opener?.associatedItem;
    }
